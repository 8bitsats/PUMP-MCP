...
export async function sellToken(tokenAddress, sellAmount = 0, accountName = "default", slippageBasisPoints = 100) {
    try {
        ...
        console.error(`Checking if keys folder exists: ${fs.existsSync(keysFolder)}`);
        ...
        const tokenBalance = await getSPLBalance(connection, mintPublicKey, account.publicKey);
        ...
        const amountToSell = sellAmount === 0 ? tokenBalance : Math.min(sellAmount, tokenBalance);
        ...
        const result = await sdk.sell(account, mintPublicKey, BigInt(amountToSell * Math.pow(10, 6)), BigInt(slippageBasisPoints), DEFAULT_PRIORITY_FEES);
        if (!result.success) {
...
async function main() {
    ...
    if (args.length < 1) {
        console.error("Usage: node sell-token.js <token_address> [sell_amount] [account_name] [slippage_basis_points]");
        console.error("Example: node sell-token.js G5e2XonmccmdKc98g3eNQe5oBYGw9m8xdMUvVtcZpump 1000 default 100");
        console.error("Note: If sell_amount is 0 or not provided, all tokens will be sold.");
        process.exit(1);
    }
    ...
    console.log(`Selling ${sellAmountText} tokens of ${tokenAddress} with account ${accountName} and slippage ${slippageBasisPoints} basis points`);
    try {
        const result = await sellToken(tokenAddress, sellAmount, accountName, slippageBasisPoints);
        console.log("\nResult:");

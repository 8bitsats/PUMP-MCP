...
export async function getAccountBalance(accountName = "default", tokenAddress) {
    try {
        ...
        if (tokenAddress) {
            ...
            const tokenBalance = await getSPLBalance(connection, mintPublicKey, keypair.publicKey);
            response.push(`Token Balance (${tokenAddress}): ${tokenBalance !== null ? tokenBalance : "No token account found"}`);
        }
        return response.join("\n");
    }
    catch (error) {
        console.error("Error getting account balance:", error);
        return `Error getting account balance: ${error?.message || "Unknown error"}`;
    }
}
async function main() {

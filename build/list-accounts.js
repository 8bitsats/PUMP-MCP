...
export async function listAccounts() {
    try {
        ...
        const accounts = files
            ...
            try {
                const keypairData = JSON.parse(fs.readFileSync(path.join(keysFolder, file), "utf-8"));
...
export function formatListAccountsResult(result) {
    if (!result.success) {
...
async function main() {
    try {
        ...
        const mcpResponse = createMcpResponse(formattedResult || "Error: No result");

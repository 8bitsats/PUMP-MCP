...
import { getTokenInfo, formatTokenInfo, createMcpResponse, } from "./get-token-info.js";
    ...
    try {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Error getting token info: ${error?.message || "Unknown error"}`,
                },
            ],
        };
    }
...
server.tool("create-token", "Create a new Pump.fun token", {
    ...
    accountName: z
        .string()
        .default("default")
        .describe("Name of the account to use (will be created if it doesn't exist)"),
}, async ({ name, symbol, description, imageUrl, initialBuyAmount, accountName, }) => {
    try {
...
server.tool("sell-token", "Sell a Pump.fun token", {
    ...
    try {
        console.error(`Selling token: ${tokenAddress}, amount: ${sellAmount === 0 ? "ALL" : sellAmount}`);
...
server.tool("list-accounts", "List all accounts in the keys folder", {}, async () => {
    try {
        console.error("Listing accounts");
        const result = await listAccounts();
        const formattedResult = formatListAccountsResult(result);
        return createMcpResponse(formattedResult || "Error: No account information available");
    }
    catch (error) {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Error listing accounts: ${error?.message || "Unknown error"}`,
                },
            ],
        };
    }
});
server.tool("get-account-balance", "Get the SOL and token balances for an account", {
    accountName: z
        .string()
        .default("default")
    ...
    try {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Error getting account balance: ${error?.message || "Unknown error"}`,
                },
            ],
        };
    }

...
export async function buyToken(tokenAddress, buyAmount, accountName = "default", slippageBasisPoints = 100) {
    try {
        ...
        if (!fs.existsSync(keysFolder)) {
            try {
                ...
                return {
                    success: false,
                    error: `Error creating keys folder: ${mkdirError.message || JSON.stringify(mkdirError)}`,
                };
            }
        }
        ...
        const requiredBalance = buyAmount * LAMPORTS_PER_SOL + 0.001 * LAMPORTS_PER_SOL;
        console.log(`Required balance: ${requiredBalance / LAMPORTS_PER_SOL} SOL`);
        if (balance < requiredBalance) {
            const errorMessage = `Insufficient SOL balance. Account ${account.publicKey.toString()} has ${balance / LAMPORTS_PER_SOL} SOL, but needs at least ${requiredBalance / LAMPORTS_PER_SOL} SOL. Please send SOL to this address and try again.`;
            console.error(errorMessage);
            return { success: false, error: errorMessage };
        }
        const mintPublicKey = new PublicKey(tokenAddress);
        console.log(`Token address: ${tokenAddress}`);
        const initialTokenBalance = (await getSPLBalance(connection, mintPublicKey, account.publicKey)) || 0;
        console.log(`Initial token balance: ${initialTokenBalance}`);
        console.log(`Buying ${buyAmount} SOL worth of tokens...`);
        const result = await sdk.buy(account, mintPublicKey, BigInt(buyAmount * LAMPORTS_PER_SOL), BigInt(slippageBasisPoints), DEFAULT_PRIORITY_FEES);
        if (!result.success) {
        ...
        const newTokenBalance = (await getSPLBalance(connection, mintPublicKey, account.publicKey)) || 0;
...
async function main() {
    ...
    if (args.length < 2) {
        console.error("Usage: node buy-token.js <token_address> <buy_amount_sol> [account_name] [slippage_basis_points]");
        console.error("Example: node buy-token.js G5e2XonmccmdKc98g3eNQe5oBYGw9m8xdMUvVtcZpump 0.1 default 100");
        process.exit(1);
    }
    ...
    if (isNaN(buyAmount) || buyAmount < 0.0001) {
        console.error("Buy amount must be a number greater than or equal to 0.0001");
        process.exit(1);
    }
    ...
    console.log(`Buying ${buyAmount} SOL worth of token ${tokenAddress} with account ${accountName} and slippage ${slippageBasisPoints} basis points`);
    try {
        const result = await buyToken(tokenAddress, buyAmount, accountName, slippageBasisPoints);
        console.log("\nResult:");

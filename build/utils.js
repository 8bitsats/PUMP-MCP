...
function getOrCreateKeypair(keysFolder, name) {
...
async function getSPLBalance(connection, mint, owner) {
    try {
...
async function printSOLBalance(connection, publicKey, label = "Account") {
    try {
...
async function printSPLBalance(connection, mint, owner, label = "SPL Token") {
    try {
        const balance = await getSPLBalance(connection, mint, owner);
        console.log(`${label} Balance:`, balance !== null ? balance : "No token account found");
    }
    catch (error) {
...
export { __filename, __dirname, rootDir, getKeysFolder, getOrCreateKeypair, getSPLBalance, printSOLBalance, printSPLBalance, safeStringify, };
